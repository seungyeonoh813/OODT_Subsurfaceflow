...
for i in range(n_test):
    ...
    # When it terminates early, padding
    while len(temp_u) < 30:
...
ax1.set_title('1. PPO Training Curve')
...
for i in range(n_test): 
    ax2.plot(time_axis, arr_u[i], 'r-', alpha=0.1)
ax2.plot(time_axis, mean_u, 'r-', linewidth=3, label='Mean U')
ax2.set_title('2. Control (injection)')
...
for i in range(n_test): 
    ax3.plot(time_axis, arr_p[i], 'b-', alpha=0.1)
